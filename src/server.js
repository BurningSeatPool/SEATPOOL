            ...
            for(var i=1;i<list.length;i++){
                ...
                const trainSale = (await driver).findElement(By.xpath(path+i+sale)).then(function (trainSale){
                    trainSale.getAttribute('alt').then(function (alt){
                        if(alt == "35%예약"){
                            tr_fair = price(from, to)*0.35;
                            tr_sale = '35%할인';
                        }else if(alt == "30%예약"){
                            tr_fair = price(from, to)*0.3;
                            tr_sale = '30%할인';
                        }else if(alt == "25%예약"){
                            tr_fair = price(from, to)*0.25;
                            tr_sale = '25%할인';
                        }else if(alt == "20%예약"){
                            tr_fair = price(from, to)*0.2;
                            tr_sale = '20%할인';
                        }else if(alt == "15%예약"){
                            tr_fair = price(from, to)*0.15;
                            tr_sale = '15%할인';
                        }
                    })
                });
                ...
                connection.query(`insert ignore into train_info (Train_ID, Depart, Dest, month, day, Depart_Time, Arrival_Time, Amount, timecode, sale) values (
                    '${tr_no}','${from}','${to}', '${month}', '${day}','${tr_start}','${tr_end}','${tr_fair}','${timecode}','${tr_sale}')`, function (error, results, fields) {
                    if (error) {
                        console.log(error);
                    }
                });
